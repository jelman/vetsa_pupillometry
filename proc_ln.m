function avgdata = proc_ln(filename,graphics,nobasecal,basestart,baselength,disblonset);

% USAGE: avgdata = proc_ln(filename,graphics,basestart,baselength,disblonset);
%
...
%
% Assumptions: spreadsheet must have "ID", "Task (DS)", and "1st data pt" columns
% Rows that have "Bad data" or "no data" in Task column will be removed from further analysis.
% Any row that has no data should be properly coded in "Task (DS)" column.
%
% Input:
...
%   disblonset = toggle 0/1 to ignore the data discarding with blink at onset
% Output:
...
% Adapted for new data with 30Hz sampling rate by Jia Guo, Ph.D. on 11/15/2014
...
if ~exist('disblonset','var')
    % switch to turn off the data discarding with blink at onset 
    disblonset = 0; % discard data with blink at onset by default
end
...
filenamesubst = sprintf('%s%s_substats.xlsx',pref,filename(1:s-1));
filenamesubavg = sprintf('%s%s_subavg.xlsx',pref,filename(1:s-1));
...
elseif strfind(header{TRcol},'DS')
    ...
    ns = 27; % 3 before and 3 after, and 8 trials
    DScol = strmatch('Bx Trial',header);
    DS = textdata(:,DScol);
    conds = ['b' 'a' '2' '3' '4' '5' '6' '7' '8' 'c'];% before, after, combined (before and after), and 7 other 
    conds_sub = {'2subc' '3subc' '4subc' '5subc' '6subc' '7subc' '8subc'}; % subtraction conditions
%     condsamp = {[30:39]; [59:68]; [88:97]}; % relevant samples for each DS
%     condsamp = {[sf*3+1:sf*4]; [sf*6+1:sf*7]; [sf*9+1:sf*10]}; % relevant samples for each DS in 3-4s, 6-7s and 9-10s
    fprintf('Processing Digit Span (letter and number) data...\n');
end
...
data.RawPupilTrials = numdata(Good,Data1col:Data1col+floor(25*sf)-1); % process all data recorded in 25s span
...
%     tempdata=puppreprocess(tempdata,.3,.2,1.0,min(floor(sf/60*10),2),min(floor(sf/60*7),2)); %adjusted for slower sampling rate
    ...
    % find the first instance of '10' and call it the end of the valid data
    ind = find(tempdata.RescaleData==10);
    if numel(ind)>0 % not all data valid
        tempdata.RescaleData = tempdata.RescaleData(1:ind(1)-1);
        tempdata = stublinks_new(tempdata,0,0,[],0.1,sf);
        data.PupilTrials(i,1:ind(1)-1) = tempdata.NoBlinks;
        data.BlinkTrials(i,1:ind(1)-1) = tempdata.BlinkTimes;
    else % all data valid
        tempdata = stublinks_new(tempdata,0,0,[],0.1,sf);
...
for c = 1:length(conds)
    ntr = sprintf('ntr%s',conds(c));
    avgdata.stats.(ntr) = zeros(length(subjects),1);
    AvgAmpBaseline = sprintf('AvgAmpBaseline%s',conds(c));
    avgdata.stats.(AvgAmpBaseline) = zeros(length(subjects),1);
    AvgAmpDS = sprintf('AvgAmpDS%s',conds(c));
    avgdata.stats.(AvgAmpDS) = zeros(length(subjects),1);
    for t = 1:25
        AvgAmpTW = sprintf('AvgAmpTW%i_%s',t,conds(c));
        avgdata.stats.(AvgAmpTW) = zeros(length(subjects),1);
    end;
    for t = 1:25
        AvgNormAmpTW = sprintf('AvgNormAmpTW%i_%s',t,conds(c));
        avgdata.stats.(AvgNormAmpTW) = zeros(length(subjects),1);
    end
end

% below are for subtraction conditions
% Will store stats in subdata structure
subdata.id = {};
subdata.Data = zeros(length(subjects),numpts,length(conds_sub));
subdata.Time = [0:1:numpts-1]./sf;
for c = 1:length(conds_sub)
    subAmpDS = sprintf('subAmpDS%s',conds_sub{c});
    subdata.stats.(subAmpDS) = zeros(length(subjects),1);
    for t = 1:25
        subAmpTW = sprintf('subAmpTW%i_%s',t,conds_sub{c});
        subdata.stats.(subAmpTW) = zeros(length(subjects),1);
    end
end
...
for s = 1:length(subjects)
    ...
    if length(ind)>1
        ...
        % drop short trials, not needed in CF task
        if strfind(header{TRcol},'PLR')
        ...
        elseif strfind(header{TRcol},'DS') % drop trials with blinks > 50%
            dropsDS = zeros(length(drops),1);
            tempdata.cond = zeros(1,length(drops));
            
            indTR = strmatch('LNSP',TR(ind)); % before and after conditions
            for i=1:numel(indTR)
                tmp = TR(ind(indTR(i)));
                switch tmp{1}(5:end)
                    case {'1','2','3'}
                        tempdata.cond(indTR(i)) = 'b'; % before
                    case {'4','5','6'}
                        tempdata.cond(indTR(i)) = 'a'; % after
                end
            end
            % drop trials with >50% blinks in the 0-9s (9*sf samples) range
            % for condition before (L8abc) and after (L8def)
            dropsDS(indTR) = sum(tempdata.BlinkTrials(indTR,1:9*sf),2)> floor(9*sf/2);
            
            indnew = setdiff(ind,ind(indTR)); % conditions other than before or after
            
            for i=1:numel(indnew)
            % fill in conditions
                indDS = find(ind==indnew(i));
                tmp = DS(indnew(i));
                tempdata.cond(indDS) = tmp{1}(2);
                % drop trials with >50% blinks in the 0-Xs (X*sf samples) range
                % for conditions 2-8 (X=3-9 respectively)
                critSec = str2num(tmp{1}(2));
                dropsDS(indDS) = sum(tempdata.BlinkTrials(indDS,1:(critSec+1)*sf),2)> floor((critSec+1)*sf/2);
            end
        ...
        if nobasecal==0 % baseline calculation required
            if disblonset==0
                ...
                fprintf('Discarding data with blink at onset disabled...\n');
                if graphics subplot(4,1,4); title('dropping data with blinks at onset disabled'); end
            end
        else
    ...
    %loop through original conditions
    for c = unique(tempdata.cond) 
%         if c=='c'
%             % find valid input waveform from 'a' and 'b'
%             indDS = find(tempdata.cond=='b');
%             good = ind(indDS(find(tempdata.drops(indDS)<1)));  %index into all data
%             bad = find(tempdata.drops(indDS)); %index into one subject data
%             indDS = find(tempdata.cond=='a');
%             good = [good ind(indDS(find(tempdata.drops(indDS)<1)))];  %index into all data
%             bad = [bad find(tempdata.drops(indDS))]; %index into one subject data
%         else
...
%         end
        ...
        if ~isempty(good) && length(good)>= 1 %accepting average for at least 1 trial for DS
            ...
            AvgAmpBaseline = sprintf('AvgAmpBaseline%s',c);
            ...
            condata = data.PupilTrials(good,:); % all data 
            v = nonzeros(condata);
            AvgAmpDS = sprintf('AvgAmpDS%s',c);
            if ~isempty(v)
                avgdata.stats.(AvgAmpDS)(s) = mean(v);
            end
            
            for t = 1:25
                AvgAmpTW = sprintf('AvgAmpTW%i_%s',t,c);
                AvgNormAmpTW = sprintf('AvgNormAmpTW%i_%s',t,c); %Amplitude normalized to baseline
    ...
    % below is for the combined condition (before and after)
    c_add = 'c';
    avgdata.id(s) = ID(ind(1));
    ...
    ntr = sprintf('ntr%s',c_add);
    avgdata.stats.(ntr)(s) = length(good);
    
    if ~isempty(good) && length(good)>= 1 %accepting average for at least 1 trial for DS
        % get averaged waveform for remaining good trials
        avgdata.ntr(s,find(conds==c_add)) = length(good);
        avgdata.NormedPupData(s,:,find(conds==c_add)) = mean(data.NormedPupTrials(good,:),1);
        avgdata.CleanPupData(s,:,find(conds==c_add)) = mean(data.PupilTrials(good,:),1);
        
        % baseline calculation
        AvgAmpBaseline = sprintf('AvgAmpBaseline%s',c_add);
        if nobasecal==1 % if baseline calculation is ignored
            avgdata.stats.(AvgAmpBaseline)(s) = 0;
        else % if baseline calculation is required
            basedata = data.PupilTrials(good,basestart:basestart+baselength-1);
        ...
        condata = data.PupilTrials(good,:); % all data
        v = nonzeros(condata);
        AvgAmpDS = sprintf('AvgAmpDS%s',c_add);
        if ~isempty(v)
            avgdata.stats.(AvgAmpDS)(s) = mean(v);
        end
        
        for t = 1:25
            AvgAmpTW = sprintf('AvgAmpTW%i_%s',t,c_add);
            AvgNormAmpTW = sprintf('AvgNormAmpTW%i_%s',t,c_add); %Amplitude normalized to baseline
            condata = data.PupilTrials(good,find(avgdata.Time>=t-1 & avgdata.Time<t));
            v = nonzeros(condata);
            if ~isempty(v)
                avgdata.stats.(AvgAmpTW)(s) = mean(v);
    ...
    % subtraction of averaged combined data below (LNS before and after)from
    % conditions of 2-8 (LNS)
    for icon = 1:7 % subtraction conditions  '2-c' '3-c' '4-c' '5-c' '6-c' '7-c' '8-c'
        c_ori = conds(icon+2);
        c_add = conds_sub{icon};
        subdata.id(s) = ID(ind(1));
        ntr = sprintf('ntr%s',num2str(icon+1));
        
        if avgdata.stats.(ntr)(s)>=1 && avgdata.stats.ntrc(s)>= 1 %accepting average if both LN and LNS_combined data are valide
            % find the min data length
            lengtha = numel(find(avgdata.CleanPupData(s,:,find(conds==c_ori))));
            lengthb = numel(find(avgdata.CleanPupData(s,:,find(conds=='c'))));
            minlength = min(lengtha,lengtha);
            
            subdata.Data(s,1:minlength,icon) = avgdata.CleanPupData(s,1:minlength,find(conds==c_ori)) - avgdata.CleanPupData(s,1:minlength,find(conds=='c'));

            v = subdata.Data(s,1:minlength,icon); % subtraction data
            subAmpDS = sprintf('subAmpDS%s',c_add);
            if ~isempty(v)
                subdata.stats.(subAmpDS)(s) = mean(v);
            end
            
            for t = 1:25
                subAmpTW = sprintf('subAmpTW%i_%s',t,c_add);
                condata = subdata.Data(s,find(subdata.Time>=t-1 & subdata.Time<t),icon);
                v = nonzeros(condata);
                if ~isempty(v)
                    subdata.stats.(subAmpTW)(s) = mean(v);
                end;
            end;
    ...
    if graphics
        ...
        for icond = 1:numel(unique(tempdata.cond)) % all valid conditions
            if ~exist('h2','var')
            ...
            for t = icond*3-2:icond*3 %size(tempdata.NormedPupTrials,1)
                isubplot = mod(t,3);
                if isubplot ==0
                    isubplot = 3;
                end
                subplot(4,1,isubplot) % each condition has 3 trials
                ...
                if ~isempty(find(tempdata.drops<1))
                    subplot(4,1,4) % show mean for each condition
...
% below are for subtraction results
% print averaged waveforms after subtraction to xls file
clear xlsdata
xlsdata(1,1:2) = {'ID' 'DS'};
xlsdata(1,3:numpts+2)=num2cell(subdata.Time);
for c = 1: length(conds_sub)
    xlsdata(1+(c-1)*(length(subdata.id))+1:1+c*(length(subdata.id)),1) = subdata.id;
    xlsdata(1+(c-1)*(length(subdata.id))+1:1+c*(length(subdata.id)),2) = conds_sub(c);
    xlsdata(1+(c-1)*(length(subdata.id))+1:1+c*(length(subdata.id)),3:numpts+2) = num2cell(subdata.Data(:,:,c));
end;
xlswrite(filenamesubavg,xlsdata);

% print stats after subtraction to xls file
clear xlsdata
stats = subdata.stats;
xlsdata(1,1) = {'ID'};
varnames = fieldnames(stats);
xlsdata(1,2:length(varnames)+1)=varnames;
xlsdata(2:length(subdata.id)+1,1) = subdata.id;
for s = 1: length(varnames)
    xlsdata(2:length(subdata.id)+1,s+1) = num2cell(getfield(stats,varnames{s})');
end
xlswrite(filenamesubst,xlsdata);
