function avgdata = proc_cf(filename,graphics,nobasecal,basestart,baselength,disblonset);

% USAGE: avgdata = proc_cf (filename,graphics,basestart,baselength);
%
...
elseif strfind(header{TRcol},'DS')
    ...
    ns = 6; % 3 phenomic and 3 semantic trials
    DScol = strmatch('Bx Trial',header);
    DS = textdata(:,DScol);
    conds = [1 2];% 1: semantic ('Ca' 'Cb' 'Cc') and 2: semantic ('Cd' 'Ce' 'Cf')
%     conds_trname = {'Ca' 'Cb' 'Cc';  % phenomic
%              'Cd' 'Ce' 'Cf' };% semantic
...
%     condsamp = {[sf*3+1:sf*4]; [sf*6+1:sf*7]; [sf*9+1:sf*10]}; % relevant samples for each DS in 3-4s, 6-7s and 9-10s
    fprintf('Processing Digit Span (category fluency) data...\n');
end
...
for s = 1:length(subjects)
    ...
    if length(ind)>1
        ...
        % drop trials with >50% blinks in the 0-25s (25*sf samples) range
        % for CF data
        drops=drops+(sum(tempdata.BlinkTrials(:,1:25*sf),2)> floor(25*sf/2));
        if graphics subplot(4,1,2); bar(sum(tempdata.BlinkTrials(:,1:25*sf),2)>floor(25*sf/2)); ylabel('> 50% blinks in 0-X critical period'); end
        ...
        elseif strfind(header{TRcol},'DS')
            ...
            indDS = strmatch('Ca',DS(ind));
            tempdata.cond(indDS) = 1;
            indDS = strmatch('Cb',DS(ind));
            tempdata.cond(indDS) = 1;
            indDS = strmatch('Cc',DS(ind));
            tempdata.cond(indDS) = 1;
            indDS = strmatch('Cd',DS(ind));
            tempdata.cond(indDS) = 2;
            indDS = strmatch('Ce',DS(ind));
            tempdata.cond(indDS) = 2;
            indDS = strmatch('Cf',DS(ind));
            tempdata.cond(indDS) = 2;
%             dropsDS(indDS) = (sum(tempdata.RawPupilTrials(indDS,condsamp{find(conds=='phenomic')})==0,2)> floor(sf/2));
...
% %             dropsDS(indDS) = (sum(tempdata.RawPupilTrials(indDS,condsamp{find(conds==9)})==0,2)> floor(sf/2));
            ...
            if graphics subplot(4,1,3); bar(dropsDS); ylabel('short trial'); end
            drops = drops+dropsDS;
        end
    ...
    for c = unique(tempdata.cond) % for phenomic and semantic
%         if c==1 % phenomic
%             indDS = find(tempdata.cond=='a' || tempdata.cond=='b' || tempdata.cond=='c');
%         else
%             % semantic
%             indDS = find(tempdata.cond=='d' || tempdata.cond=='e' || tempdata.cond=='f');
%         end
        ...
        if ~isempty(good) && length(good)>= 1 %accepting average for at least 1 trial for DS
            ...
            condata = data.PupilTrials(good,:); % no critical sampling period for cf data
            v = nonzeros(condata);
    ...
    if graphics
        ...
        for icond = 1:2 % 2 conditions (phenomic and semantic) in CF experiment
            if ~exist('h2','var')
